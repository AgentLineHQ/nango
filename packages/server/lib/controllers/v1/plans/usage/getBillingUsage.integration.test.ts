...
describe(`GET ${route}`, () => {
    ...
    describe('ClickHouse happy path (source=clickhouse)', () => {
        ...
        it('rejects filter + breakdown on the SAME dimension (degenerate single-value split)', async () => {
            const { apiKey } = await seedAccount();
            const res = await api.fetch(route, {
                token: apiKey.secret,
                query: {
        ...
        it('filter matches zero rows + breakdown — 200 with an empty breakdown, AVG stays cumulative', async () => {
            const { apiKey } = await seedAccount();
            const res = await api.fetch(route, {
                token: apiKey.secret,
                query: {
