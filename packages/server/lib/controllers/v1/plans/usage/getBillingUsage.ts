...
const querySchema = z
    .object({
        ...
        // May combine with `breakdown[<metric>]` on the same metric as long as
        // the dimensions differ (drill-in: filter to one value, then re-break-
        // down by another dim); only the degenerate same-dim pairing is rejected
        // by the refine below. CH path only.
        filter: filterSchema
    })
    .refine(
        (data) => {
